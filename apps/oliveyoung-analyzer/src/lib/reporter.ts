...
import { formatKeyword, formatCategory } from './keywords';
...
// 체계적 폴더 경로 생성
export function createOrganizedPath(productName: string, baseDir: string): string {
...
`;
  ...
  keywords.positive.forEach((kw, i) => {
    report += `| ${i + 1} | **${formatKeyword(kw.keyword)}** | ${kw.count.toLocaleString()}건 | ${kw.percentage}% | ${formatCategory(kw.category)} |\n`;
  });
...
`;
  ...
  keywords.positive.forEach(kw => {
    if (!positiveByCategory[kw.category]) positiveByCategory[kw.category] = [];
    positiveByCategory[kw.category].push(formatKeyword(kw.keyword));
  });

  Object.entries(positiveByCategory).forEach(([cat, kws]) => {
    report += `- **${formatCategory(cat)}**: ${kws.slice(0, 4).join(', ')}\n`;
  });
...
`;
  ...
  improvements.forEach(imp => {
    report += `| ${formatCategory(imp.issue)} | ${imp.frequency}건 | ${imp.opportunity} | ${imp.suggestedAction} |\n`;
  });
...
### 강점 (Strengths)
${keywords.positive.slice(0, 5).map(k => `- ${formatKeyword(k.keyword)} (${k.percentage}%)`).join('\n')}

### 개선점 (Areas for Improvement)
${keywords.negative.slice(0, 5).map(k => `- ${formatKeyword(k.keyword)} (${k.percentage}%)`).join('\n')}
...
// 한 줄 요약 생성
function generateOneLiner(avgRating: number, keywords: BasicAnalysis['keywords']): string {
  const topStrength = keywords.positive[0] ? formatKeyword(keywords.positive[0].keyword) : '만족도';
  const topWeakness = keywords.negative[0] ? formatKeyword(keywords.negative[0].keyword) : '개선점';
...
export function saveResults(
  crawlResult: CrawlResult,
  baseDir: string
): { jsonPath: string; csvPath: string; organizedDir: string } {
  ...
  const csvRows = crawlResult.reviews.map(r =>
