...
exports.analyzeImprovementOpportunities = analyzeImprovementOpportunities;
const keywords_1 = require("./keywords");
// 리뷰 데이터 기본 분석
function analyzeReviews(crawlResult) {
...
// 옵션별 분석
function analyzeByOption(reviews) {
...
// 피부타입 분포
function analyzeSkinTypes(reviews) {
    const skinMap = {};
    reviews.forEach(r => {
        const skinType = (0, keywords_1.getSkinTypeName)(r.skinType);
        skinMap[skinType] = (skinMap[skinType] || 0) + 1;
    });
    return skinMap;
}
// 키워드 분석
function analyzeKeywords(reviews) {
    const totalReviews = reviews.length;
    const positiveCount = {};
    const negativeCount = {};
    const allPositive = (0, keywords_1.getAllPositiveKeywords)();
    const allNegative = (0, keywords_1.getAllNegativeKeywords)();
    reviews.forEach(r => {
        ...
        allNegative.forEach(keyword => {
            if (content.includes(keyword) && !(0, keywords_1.isNegativeInPositiveContext)(content, keyword)) {
                negativeCount[keyword] = (negativeCount[keyword] || 0) + 1;
            }
        });
    });
    const positive = Object.entries(positiveCount)
        .map(([keyword, count]) => ({
        keyword,
        count,
        percentage: Math.round((count / totalReviews) * 1000) / 10,
        category: (0, keywords_1.findKeywordCategory)(keyword, true)
    }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 15);
    const negative = Object.entries(negativeCount)
        .map(([keyword, count]) => ({
        keyword,
        count,
        percentage: Math.round((count / totalReviews) * 1000) / 10,
        category: (0, keywords_1.findKeywordCategory)(keyword, false)
    }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 15);
...
// 하이라이트 문구 추출
function extractHighlightQuotes(reviews) {
    const quotes = [];
    ...
    topReviews.forEach(r => {
        ...
        const highlights = sentences.filter(s => s.includes('좋아') || s.includes('최고') || s.includes('대박') ||
...
// 개선 기회 분석
function analyzeImprovementOpportunities(crawlResult, analysis) {
    const { reviews } = crawlResult;
    const opportunities = [];
    const categoryIssues = {};
    Object.entries(keywords_1.NEGATIVE_KEYWORDS).forEach(([category, keywords]) => {
            ...
            keywords.forEach(keyword => {
                if (content.includes(keyword) && !(0, keywords_1.isNegativeInPositiveContext)(content, keyword)) {
                    ...
                    if (categoryIssues[category].samples.length < 3) {
                        ...
                        if (sentence)
                            categoryIssues[category].samples.push(sentence.trim());
                    }
                }
            });
        });
    });
    const issueToOpportunity = {
        effect: {
