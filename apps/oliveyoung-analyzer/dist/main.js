...
async function ensurePlaywrightBrowser() {
    ...
    return new Promise((resolve) => {
        ...
        try {
            ...
            dialog.showErrorBox('설치 실패 (Installation Failed)', `브라우저 설치에 실패했습니다.\n(Browser installation failed.)\n\n수동 설치: npx playwright install chromium\n\n${error.message}`);
            resolve(false);
        }
    });
}
...
// IPC 핸들러: 분석 실행
ipcMain.handle('analyze', async (_event, input) => {
    const goodsNo = (0, crawler_1.extractGoodsNo)(input);
    if (!goodsNo) {
    ...
    try {
        // 1. 크롤링
        const crawlResult = await (0, crawler_1.crawlReviews)(goodsNo, sendProgress);
        if (crawlResult.reviews.length === 0) {
            return { success: false, error: '리뷰를 수집하지 못했습니다.' };
        }
        // 2. 분석
        sendProgress('리뷰 분석 중...', 92);
        const analysis = (0, analyzer_1.analyzeReviews)(crawlResult);
        const improvements = (0, analyzer_1.analyzeImprovementOpportunities)(crawlResult, analysis);
        // 3. 저장 - 체계적 폴더 구조 사용
        sendProgress('파일 저장 중...', 95);
        const baseDir = path.join(app.getPath('documents'), '올리브영분석');
        const { jsonPath, csvPath, organizedDir } = (0, reporter_1.saveResults)(crawlResult, baseDir);
        const reportPath = (0, reporter_1.generateMarkdownReport)({
            crawlResult,
            analysis,
            improvements,
        ...
        sendProgress('완료!', 100);
        // 감성 분석 계산
        const positiveCount = crawlResult.reviews.filter((r) => r.rating >= 4).length;
        const negativeCount = crawlResult.reviews.filter((r) => r.rating <= 2).length;
        const neutralCount = crawlResult.reviews.length - positiveCount - negativeCount;
        // 피부고민 분석
        const skinConcerns = {};
        crawlResult.reviews.forEach((r) => {
            if (r.skinConcern) {
        ...
        Object.keys(keywords_1.CATEGORY_TRANSLATIONS).forEach(cat => {
            const catKeywords = allKeywords
                .filter((kw) => kw.category === cat)
                .map((kw) => ({
                ...kw,
                formatted: (0, keywords_1.formatKeyword)(kw.keyword)
            }));
            if (catKeywords.length > 0) {
                categories[cat] = {
                    name: (0, keywords_1.formatCategory)(cat),
                    items: catKeywords
                };
            }
        });
        // 개선 기회 정리
        const improvementsList = improvements.slice(0, 4).map((imp) => ({
        ...
        return {
            ...
            data: {
                ...
                analysis: {
                    ...
                    keywords: {
                        positive: analysis.keywords.positive.slice(0, 10).map((kw) => ({
                            ...kw,
                            formatted: (0, keywords_1.formatKeyword)(kw.keyword)
                        })),
                        negative: analysis.keywords.negative.slice(0, 10).map((kw) => ({
                            ...kw,
                            formatted: (0, keywords_1.formatKeyword)(kw.keyword)
                        }))
                    },
