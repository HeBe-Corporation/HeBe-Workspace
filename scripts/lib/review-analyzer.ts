...
function calculateStats(reviews: Review[]): BasicAnalysis['stats'] {
  ...
  const repurchaseRate = (repurchaseCount / totalReviews) * 100;

  // 사진 포함율
  const photoCount = reviews.filter(r => r.hasPhoto).length;
  const photoRate = (photoCount / totalReviews) * 100;
  ...
  return {
    ...
    repurchaseCount,
    photoRate: Math.round(photoRate * 10) / 10,
    photoCount
  };
}
...
function analyzeByOption(reviews: Review[]): OptionAnalysis[] {
  ...
  return Array.from(optionMap.entries())
    .map(([name, data]) => ({
      ...
      percentage: Math.round((data.count / totalReviews) * 1000) / 10
    }))
...
/**
 * 옵션별 개선점 분석 (평점이 낮은 옵션 찾기)
 */
export function findWeakOptions(analysis: BasicAnalysis): OptionAnalysis[] {
  const avgRating = analysis.optionAnalysis.reduce((sum, o) => sum + o.avgRating, 0) /
                    analysis.optionAnalysis.length;

  return analysis.optionAnalysis
    .filter(o => o.avgRating < avgRating && o.count >= 10)
