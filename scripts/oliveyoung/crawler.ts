...
import { oliveyoung as outputOrganizer, ensureOutputDir, getOutputPath } from '../lib/output-organizer';
 ...
 * 결과 저장 (자동 폴더 정리 적용)
 */
function saveResults(result: CrawlResult): { jsonPath: string; csvPath: string } {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const baseFilename = `${result.goodsNumber}-full-${timestamp}`;

  // JSON 저장 (output-organizer 사용)
  const jsonPath = getOutputPath('oliveyoung', 'data', baseFilename, 'json');
  ensureOutputDir(jsonPath);
  fs.writeFileSync(jsonPath, JSON.stringify(result, null, 2), 'utf-8');

  // CSV 저장
  const csvPath = getOutputPath('oliveyoung', 'data', baseFilename, 'csv');
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('사용법: npx ts-node scripts/oliveyoung/crawler.ts <URL 또는 상품번호>');
    process.exit(1);
  }
  ...
  // 저장 (자동 폴더 정리됨: output/oliveyoung/YYYY-MM/)
  const { jsonPath, csvPath } = saveResults(crawlResult);
  console.log(`\n📁 저장 완료:`);
  ...
  // 리포트 생성 (output/oliveyoung/YYYY-MM/ 폴더에 저장)
