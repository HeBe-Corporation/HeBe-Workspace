 ...
 * Windows 환경용 TypeScript 버전
 *
 * 실행: npx ts-node cleanser_research.ts
 */
...
async function main() {
    ...
    console.log('============================================');
    console.log('🧴 Skin Nerd 클렌저 집중 48시간 리서치');
    console.log('============================================');
    console.log(`시작: ${new Date().toLocaleString()}`);
    console.log(`종료 예정: ${new Date(endTime).toLocaleString()}`);
    console.log(`질문 주기: ${INTERVAL_MINUTES}분`);
    console.log(`총 질문: ${QUESTIONS.length}개`);
    console.log('============================================\n');

    // 성분 리서치 파일 초기화
    const ingredientsFile = path.join(REPORT_DIR, `cleanser_ingredients_${TIMESTAMP}.md`);
    fs.writeFileSync(ingredientsFile, `# Skin Nerd 클렌저 성분(Ingredients) 리서치\n생성일시: ${new Date().toLocaleString()}\n\n---\n\n`);

    while (Date.now() < endTime) {
        const currentQuestion = QUESTIONS[questionIndex];
        const logFile = path.join(LOG_DIR, `cleanser_${TIMESTAMP}_${String(iteration).padStart(3, '0')}.md`);

        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log(`🔬 #${iteration} | ${new Date().toLocaleTimeString()} | Q${questionIndex + 1}/${QUESTIONS.length}`);
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

        // 로그 파일 헤더
        let logContent = `# 클렌저 리서치 #${iteration}\n`;
        logContent += `- 시간: ${new Date().toLocaleString()}\n`;
        logContent += `- 질문: ${questionIndex + 1}/${QUESTIONS.length}\n\n`;
        logContent += `## 질문\n${currentQuestion}\n\n`;
        logContent += `## 응답\n\n`;

        // Claude 호출
        const response = callClaude(currentQuestion);
        logContent += response;
        ...
        // 성분 파일에 누적
        fs.appendFileSync(ingredientsFile, `## Q${questionIndex + 1}: ${currentQuestion.slice(0, 50)}...\n\n${response}\n\n---\n\n`);

        console.log(`✅ 저장: ${path.basename(logFile)}`);
        ...
        // 대기
        if (Date.now() < endTime) {
            console.log(`⏳ ${INTERVAL_MINUTES}분 대기...\n`);
            await sleep(INTERVAL_MINUTES * 60 * 1000);
        }
    }

    // 최종 리포트 생성
    const finalReport = path.join(REPORT_DIR, `cleanser_final_report_${TIMESTAMP}.md`);
    let reportContent = `# Skin Nerd 클렌저 48시간 리서치 최종 보고서\n`;
    reportContent += `- 생성: ${new Date().toLocaleString()}\n`;
    reportContent += `- 총 질문: ${iteration - 1}회\n`;
    reportContent += `- 기간: ${DURATION_HOURS}시간\n\n---\n\n`;

    const logFiles = fs.readdirSync(LOG_DIR)
        .filter(f => f.startsWith(`cleanser_${TIMESTAMP}`))
        .sort();
    ...
    fs.writeFileSync(finalReport, reportContent);

    console.log('\n============================================');
    console.log('🏁 48시간 리서치 완료!');
    console.log(`📊 최종 보고서: ${finalReport}`);
    console.log(`🧪 성분 리서치: ${ingredientsFile}`);
    console.log('============================================');
}
